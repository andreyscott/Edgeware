          ...
          <div className="flex gap-1 overflow-x-auto py-3 scrollbar-hide">
            {offeringDetails.map((o) => (
              <a
                key={o.id}
                href={`#${o.id}`}
                className="flex-shrink-0 px-4 py-2 text-xs font-semibold text-brand-navy hover:text-brand-gold hover:bg-brand-cream rounded-lg transition-all duration-200"
              >
                {o.icon} {o.title}
              </a>
            ))}
                        ...
                        <ul className="space-y-3">
                          {offering.outcomes.map((outcome) => (
                            <li key={outcome} className="flex items-start gap-3">
                              <div className="flex-shrink-0 w-5 h-5 rounded-full bg-brand-gold/20 border border-brand-gold/40 flex items-center justify-center mt-0.5">
